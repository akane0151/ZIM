...
class Settings extends CI_Controller
{
    ...
    public function change_password(){
        try{
            ...
            if($this->User_model->checkpass($this->session->userdata('userid'),$oldpassword)&& $newpassword == $confirmpassword){

                $data = array (
                    'password' => $newpassword,
                );
                $result = $this->User_model->update($data,$this->session->userdata('userid'));
                if(!is_string($result) && $result)
                    echo json_encode("true");
                elseif(is_string($result))
                    echo $result;
